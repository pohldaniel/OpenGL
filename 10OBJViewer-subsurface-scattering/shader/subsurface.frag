...
uniform vec4 u_diffuseColor = vec4(0.2, 0.7, 0.5, 1.0);
uniform vec4 u_scatterColor = vec4(0.8, 0.8, 0.0, 1.0);
uniform vec3 u_lightDirection = normalize(vec3(1.0, 0.0, 4.0));
uniform sampler2D u_texture;

uniform vec2 u_nearFar = vec2(1.0, 100.0);
uniform float u_wrap = 0.1;
uniform float u_scatterWidth = 0.5;
uniform float u_scatterFalloff = 5.0;
...
void main(void){
   ...
   float zInWC = texture(u_texture, v_projCoord.xy / v_projCoord.w).r;
	...
	float NdotL = dot(normal, normalize(v_lightdirection));		
