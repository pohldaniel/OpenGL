...
void main(void){
	...
	vec3 bitangent = normalize((u_normalMatrix * vec4(i_bitangent, 0.0)).xyz)*i_tangent.w;
	...
	mat3 TBN2 = mat3(tangent.x, tangent.y, tangent.z,
	...
	for(int i = 0; i < 2; i++){
	 vertexLight[i] =  TBN *((u_modelViewLight * vec4(u_lightPos[i], 1.0)).xyz - vertexInCamSpace);
	}
