...
Shader::Shader(){
	...
	m_program = createProgram("shader/texture.vert", "shader/texture.frag");
...
void Shader::getUniformLocation(){
	...
	u_normalMatrix = glGetUniformLocation(m_program, "u_normalMatrix");
	u_active = glGetUniformLocation(m_program, "active");
	u_modelViewLight = glGetUniformLocation(m_program, "u_modelViewLight");
}
...
void Shader::loadMatrix(GLuint location, const Matrix4f matrix){

	glUniformMatrix4fv(location, 1, true, &matrix[0][0]);

}

void Shader::loadVector(GLuint location,  Vector3f vector){

	glUniform3fv(location, 1, &vector[0]);
}

void Shader::loadFloat2d(GLuint location, float value[2]){

	glUniform1fv(location, 2, value);
}

void Shader::loadProjectionMatrix(const Matrix4f projectionMatrix){

	glUseProgram(m_program);
	glUniformMatrix4fv(u_projection, 1, true, &projectionMatrix[0][0]);
	glUseProgram(0);
}

void Shader::loadViewMatrix(const Matrix4f viewMatrix){

	glUniformMatrix4fv(u_view, 1, true, &viewMatrix[0][0]);

}

void Shader::loadModelViewMatrix(const Matrix4f modelViewMatrix){

	glUniformMatrix4fv(u_modelView, 1, true, &modelViewMatrix[0][0]);
	
}

void Shader::loadNormalMatrix(const Matrix4f normalMatrix){

	loadMatrix(u_normalMatrix, normalMatrix);
}

void Shader::loadModelViewLightMatrix(const Matrix4f modelViewMatrix){

	glUniformMatrix4fv(u_modelViewLight, 1, true, &modelViewMatrix[0][0]);
...
void Shader::loadMaterial(Material &material){
	...
	glUniform1f(glGetUniformLocation(m_program, "material.specularShininesse"), material.specularShininesse);
	glUseProgram(0);
}
...
PhongShader::PhongShader() : Shader("shader/phongTexture.vert", "shader/phongTexture.frag"){
	
}

PhongShader::PhongShader(std::string vertex, std::string fragment) : Shader(vertex, fragment){
...
NormalShader::NormalShader() : PhongShader("shader/normal.vert", "shader/normal.frag"){

	getAttributeGetAttribLocation();
	loadSampler();
}

NormalShader::NormalShader(std::string vertex, std::string fragment) : PhongShader(vertex, fragment){
...
void NormalShader::getAttributeGetAttribLocation(){
