...
void glClearBufferfv(GLenum buffer, GLint drawbuffer, const GLfloat* value) {
	...
	static PFNGLCLEARBUFFERFVPROC glClearBufferfv = 0;
	LOAD_ENTRYPOINT("glClearBufferfv", glClearBufferfv, PFNGLCLEARBUFFERFVPROC);
...
void glClampColor(GLenum target, GLenum clamp) {
	...
	LOAD_ENTRYPOINT("glClampColor", glClampColor, PFNGLCLAMPCOLORPROC);
	glClampColor(target, clamp);
}

void glDepthRangef(GLfloat nearVal, GLfloat farVal) {

	typedef void(APIENTRY * PFNGLDEPTHRANGEFPROC) (GLfloat nearVal, GLfloat farVal);
	static PFNGLDEPTHRANGEFPROC  glDepthRangef = 0;
	LOAD_ENTRYPOINT("glDepthRangef", glDepthRangef, PFNGLDEPTHRANGEFPROC);
