...
void AnimationShader::update(const AnimatedModel& model, const Camera& camera, const Entity3D& entity, std::vector<Matrix4f> jointVector) {

	loadMatrixArray("jointTransforms", jointVector, jointVector.size() < MAX_JOINTS ? jointVector.size() : MAX_JOINTS);
	loadMatrix("u_model", model.getTransformationMatrix() * entity.getWorldMatrix());
	loadMatrix("u_view", camera.getViewMatrix());
	loadMatrix("u_projection", camera.getProjectionMatrix());
}
