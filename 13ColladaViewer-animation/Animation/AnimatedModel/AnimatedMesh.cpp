...
SkeletonAnimatedMesh::SkeletonAnimatedMesh(ColladaLoader loader){
	...
	loader.loadData(positions, texCoords, normals, indices, jointIds, jointWeights, m_jointsList);
	//loader.createJoints(rootJoint);

	//std::cout << rootJoint.name << "  " << rootJoint.children.size() << std::endl;
...
SkeletonAnimatedMesh::SkeletonAnimatedMesh(std::vector<Vector3f> positions,
	std::vector<Vector3f> normals,
	std::vector<Vector2f> texCoords,
	std::vector<unsigned int> indices,
	std::vector<std::array<unsigned int, 4>> jointIds,
...
SkeletonAnimatedMesh ::~SkeletonAnimatedMesh() {
	glDeleteVertexArrays(1, &m_vao);
}

void SkeletonAnimatedMesh::draw() {
...
std::vector<Matrix4f> SkeletonAnimatedMesh::getBoneArray() {
...
void SkeletonAnimatedMesh::addJointsToArray(Joint rootJoint, std::vector<Matrix4f> &boneArray) {
...
void SkeletonAnimatedMesh::applyPoseToJoints(std::unordered_map<std::string, Matrix4f> currentPose) {
	applyPoseToJoints(currentPose, rootJoint, Matrix4f::IDENTITY);
}

void SkeletonAnimatedMesh::applyPoseToJoints(std::unordered_map<std::string, Matrix4f> currentPose, Joint &joint, Matrix4f parentTransform) {
