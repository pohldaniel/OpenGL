...
void initApp(HWND hWnd)
{
	...
	//camera->lookAt(Vector3f(0, 2, 5), Vector3f(0, 0, 0), Vector3f(0, 1, 1));
	...
	for (int i = 0; i < model->numberOfMeshes(); i++){
		...
		if (model->getMltPath() == ""){
			...
			model->getMesches()[i]->setColor(Vector3f(1.0/(i + 1) , 1.0/(i + 1) , 1.0/(i+1)));
		}
...
void renderFrame(){
	...
	glUniformMatrix4fv(u_normalMatrix, 1, true, &getNormalMatrix(modelView)[0][0]);
...
Matrix4f &getNormalMatrix(const Matrix4f &modelViewMatrix){
	...
	tmp[0][0] = (modelViewMatrix[1][1] * modelViewMatrix[2][2] - modelViewMatrix[1][2] * modelViewMatrix[2][1]) * invDet;
	tmp[0][1] = (modelViewMatrix[1][2] * modelViewMatrix[0][2] - modelViewMatrix[2][2] * modelViewMatrix[1][0]) * invDet;
	tmp[0][2] = (modelViewMatrix[1][0] * modelViewMatrix[2][1] - modelViewMatrix[1][1] * modelViewMatrix[2][0]) * invDet;
	tmp[0][3] = 0.0;

	tmp[1][0] = (modelViewMatrix[0][2] * modelViewMatrix[2][1] - modelViewMatrix[0][1] * modelViewMatrix[2][2]) * invDet;
	tmp[1][1] = (modelViewMatrix[0][0] * modelViewMatrix[2][2] - modelViewMatrix[0][2] * modelViewMatrix[2][0]) * invDet;
	tmp[1][2] = (modelViewMatrix[0][1] * modelViewMatrix[2][0] - modelViewMatrix[2][1] * modelViewMatrix[0][0]) * invDet;
	tmp[1][3] = 0.0;

	tmp[2][0] = (modelViewMatrix[0][1] * modelViewMatrix[1][2] - modelViewMatrix[1][1] * modelViewMatrix[0][2]) * invDet;
	tmp[2][1] = (modelViewMatrix[0][2] * modelViewMatrix[1][0] - modelViewMatrix[0][0] * modelViewMatrix[1][2]) * invDet;
	tmp[2][2] = (modelViewMatrix[0][0] * modelViewMatrix[1][1] - modelViewMatrix[1][0] * modelViewMatrix[0][1]) * invDet;
	tmp[2][3] = 0.0;
