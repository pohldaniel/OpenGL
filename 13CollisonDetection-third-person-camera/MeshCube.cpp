	...
	for (unsigned int v = 0; v < m_vResolution; v++) {
		...
		for (unsigned int u = 0; u < 4 * m_uResolution * (m_vResolution + 1); u++) {
			...
			if (u == ((m_uResolution - 1) + face * (m_uResolution * (1 + m_vResolution)))) {
				...
				indexBuffer.push_back(u + (1 + v) * m_uResolution);
				indexBuffer.push_back(u + v * (m_uResolution));
				indexBuffer.push_back((m_uResolution * (v + 1 + m_vResolution) + u + 1) % (4 * m_uResolution * (m_vResolution + 1)));

				indexBuffer.push_back(u + v * (m_uResolution));
				indexBuffer.push_back((m_uResolution * (v + m_vResolution) + u + 1) % (4 * m_uResolution * (m_vResolution + 1)));
				indexBuffer.push_back((m_uResolution * (v + 1 + m_vResolution) + u + 1) % (4 * m_uResolution * (m_vResolution + 1)));
				...
				indexBuffer.push_back(u + m_uResolution + 1);							

				indexBuffer.push_back(u + m_uResolution + 1);
				indexBuffer.push_back(u + m_uResolution);				
				indexBuffer.push_back(u);	
	...
	for (unsigned short face = 0; face < 2; face++) {
		...
		for (int i = k; i < k + m_vResolution; i++) {
			for (int j = k; j < k + m_uResolution; j++) {
				if (face % 2 == 0) {
					indexBuffer.push_back((i - k) * (m_uResolution + 1) + j);
					indexBuffer.push_back((i - k) * (m_uResolution + 1) + j + 1);
					indexBuffer.push_back((i - k + 1) * (m_uResolution + 1) + j);

					indexBuffer.push_back((i - k + 1) * (m_uResolution + 1) + j);
					indexBuffer.push_back((i - k) * (m_uResolution + 1) + j + 1);
					indexBuffer.push_back((i - k + 1) * (m_uResolution + 1) + j + 1);
				}else {
					indexBuffer.push_back((i - k) * (m_uResolution + 1) + j);
					indexBuffer.push_back((i - k + 1) * (m_uResolution + 1) + j);
					indexBuffer.push_back((i - k) * (m_uResolution + 1) + j + 1);
	...
	for (short face = 0; face < 6; face++) {
		...
		for (int v = 0; v < m_vResolution; v++) {
			for (int u = 0; u < m_uResolution; u++) {
				...
				if (face % 2 == 0) {
					indexBuffer.push_back(k + (v * (m_uResolution + 1) + u));
					indexBuffer.push_back(k + (v * (m_uResolution + 1) + u + 1));
					indexBuffer.push_back(k + ((v + 1) * (m_uResolution + 1) + u + 1));

					indexBuffer.push_back(k + (v * (m_uResolution + 1) + u));
					indexBuffer.push_back(k + ((v + 1) * (m_uResolution + 1) + u + 1));
					indexBuffer.push_back(k + ((v + 1) * (m_uResolution + 1) + u));
				}else {
					indexBuffer.push_back(k + (v * (m_uResolution + 1) + u + 1));
					indexBuffer.push_back(k + (v * (m_uResolution + 1) + u));
					indexBuffer.push_back(k + ((v + 1) * (m_uResolution + 1) + u + 1));
