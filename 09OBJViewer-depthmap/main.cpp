...
void initApp(HWND hWnd)
{
	...
	object = new Object();
	object->initModel("objs/face/face.obj");
	object->initShader( );
	object->m_model->scale(0.05, 0.05, 0.05);

	objects.push_back(object);

	depthmap = new Depthmap(camera);
	depthmap->setProjectionMatrix(45.0f, (GLfloat)width / (GLfloat)height, 1.0f, 2000.0f);
	depthmap->setViewMatrix(Vector3f(1.0, 0.0, 4.0), Vector3f(1.0, 0.0, 0.0), Vector3f(0.0, 1.0, 0.0));
...
void render(){
	...
	//depthmap->translate(0.0, 0.0, 0.01 );
	depthmap->rotate(Vector3f(0.0, 1.0, 0.0), 0.5);
	depthmap->renderToDepthTexture(objects[0]);
